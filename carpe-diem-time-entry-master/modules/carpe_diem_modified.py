# -*- coding: utf-8 -*-
"""
Created on Wed Jul 29 14:41:24 2020

@author: aarment

This file is the reformatted version of the python2 carpe_diem code
This code is designed to work in conjunction with to_csv.py
 - use to_csv to choose the excel file and convert the entries to csv
 - use this file to submit the csv entries to carpe_diem
"""
import os
import dotenv
...
def submit_time_entry(entry):
    ...
    # submit request
    try:
        req = urllib2.Request(os.getenv('url'), data_string, headers)
...
def prepare_data_string(entry):
    ...
    data_string += '&id=' + os.getenv('id')
    data_string += '&tbcl=N&udid=' + os.getenv('udid')
    data_string += '&date=' + date + '&key=' + os.getenv('key') + '&guid=' + str(uuid.uuid4()).upper()
    data_string += '&desc=' + urllib.quote(entry['Description'])
    data_string += '&dev=' + urllib.quote(os.getenv('dev'))
    return data_string
